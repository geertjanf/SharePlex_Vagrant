...
rm -Rf kafka_2.13-3.4.0
rm -Rf kafka-logs
rm -Rf zookeeper

#Kafka download
wget https://dlcdn.apache.org/kafka/3.4.0/kafka_2.13-3.4.0.tgz > /dev/null 2>&1
r=$?
msg="Download kafka binaries"
verify $msg $r


tar -xzf kafka_2.13-3.4.0.tgz > /dev/null 2>&1
r=$?
msg="untar kafka binaries"
verify $msg $r


ssh oracle@ol7-19-splex1 'echo -e "deactivate config kafka.cfg\n" | /u01/app/quest/prod/bin/sp_ctrl' > /dev/null 2>&1
r=$?
msg="Deactivate kafka shareplex config"
verify $msg $r
sleep 5

ssh oracle@ol7-19-splex1 'echo -e "activate config kafka.cfg\n" | /u01/app/quest/prod/bin/sp_ctrl' > /dev/null 2>&1
r=$?
