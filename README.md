...
The Vagrant scripts here will allow you to build 2 virtual machines in your computer with an Oracle Database 19c and **Shareplex 10.2** in each node (1 and 2) by just starting the VMs in the correct order.
...
```
git clone https://github.com/geertjanf/SharePlex_Vagrant.git
```

Copy the software under the "SharePlex_Vagrant" directory.

Or download the SharePlex_Vagrant-main.zip file in a directory and unzip.
...
# POC
The scripts contains severals POC's.
...
Tables created in test schema in Node 1 in pdb1 database will be replicated to test schema in target database (Node 2). 
A new schema in target is created (cdc schema) that contains all modifications from tables cdc and cdc2 in the test schema.
...
mkdir: cannot create directory ‘/u01/app/oracle/product/19/dbhome_1/datapump’: File exists

Export: Release 19.0 - Production on Thu May 23 07:15:25 2019

Copyright (c) 1982, 2014, Oracle and/or its affiliates.  All rights reserved.

Connected to: Oracle Database 12c Enterprise Edition Release 19.0 - 64bit Production
...
Dump file set for SYSTEM.EXPORT_QUEST_PERF is:
  /u01/app/oracle/product/19/dbhome_1/datapump/sample.dmp
...
The scripts will download, install and start Kafka software in Node 2.
A test is done from KAFKA user in Node 1 Oracle database to Node 2.
